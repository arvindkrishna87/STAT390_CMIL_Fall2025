...
class AttentionPool(nn.Module):
    """
    Attention pooling mechanism for MIL with elastic-net on logits (pre-softmax).
    Pools patch-level features into single bag-level representation
    """
    def __init__(self, input_dim: int, hidden_dim: int = 128,
                 l1_lambda: float = 0.0, l2_lambda: float = 0.0, dropout: float = 0.0):
        super().__init__()
    ...
    def forward(self, x: torch.Tensor, return_weights: bool = False):
        ...
        Returns:
            ...
            penalty: scalar tensor (if return_weights=True) — elastic-net on logits
        """
        # logits: (B, M)
        logits = self.attention(x).squeeze(-1)  

        # elastic-net penalty computed on logits (pre-softmax)
        penalty = (
            self.l1_lambda * torch.sum(torch.abs(logits))
            + self.l2_lambda * torch.sum(logits * logits)
        )

        # normalize AFTER computing the penalty
        weights = torch.softmax(logits, dim=1)  # (B, M)

        # weighted sum: need to expand weights to (B, M, 1)
        weighted_x = (weights.unsqueeze(-1) * x).sum(dim=1)  # (B, D)
        
        if return_weights:
            return weighted_x, weights, penalty
        return weighted_x   
...
class HierarchicalAttnMIL(nn.Module):
    ...
    def __init__(self, base_model=None, num_classes: int = 2, embed_dim: int = 512, dropout: float = 0.3):
        ...
        # Three levels of attention with dropout and elastic-net hyperparams from MODEL_CONFIG
        self.patch_attention = AttentionPool(
            embed_dim,
            MODEL_CONFIG['attention_hidden_dim'],
            l1_lambda=MODEL_CONFIG.get('attn_l1', 0.0),
            l2_lambda=MODEL_CONFIG.get('attn_l2', 0.0),
            dropout=dropout
        )
        self.stain_attention = AttentionPool(
            embed_dim,
            MODEL_CONFIG['attention_hidden_dim'],
            l1_lambda=MODEL_CONFIG.get('attn_l1_stain', MODEL_CONFIG.get('attn_l1', 0.0)),
            l2_lambda=MODEL_CONFIG.get('attn_l2_stain', MODEL_CONFIG.get('attn_l2', 0.0)),
            dropout=dropout
        )
        self.case_attention = AttentionPool(
            embed_dim,
            MODEL_CONFIG['attention_hidden_dim'],
            l1_lambda=MODEL_CONFIG.get('attn_l1_case', MODEL_CONFIG.get('attn_l1', 0.0)),
            l2_lambda=MODEL_CONFIG.get('attn_l2_case', MODEL_CONFIG.get('attn_l2', 0.0)),
            dropout=dropout
        )
    ...
    def process_single_stain(self, slice_list: List[torch.Tensor], stain_name: str, 
        ...
        for slice_tensor in slice_list:
            ...
            if return_attn_weights:
                slice_emb, patch_weights, patch_penalty = self.patch_attention(
                    patch_embeddings.unsqueeze(0), return_weights=True
                )
                # keep penalty (do NOT detach) so gradients flow back through attention
                slice_patch_penalties.append(patch_penalty)
                # weights for visualization can be detached
        ...
        if slice_embeddings:
            ...
            if return_attn_weights:
                stain_emb, stain_weights, stain_penalty = self.stain_attention(
                ...
                stain_attention_info = {
                    ...
                    'patch_penalties': slice_patch_penalties,  # list of tensors (kept attached to graph)
                    'stain_penalty': stain_penalty            # tensor (kept attached)
                }
    ...
    def forward(self, stain_slices_dict: Dict[str, List[torch.Tensor]], 
        ...
        if not stain_embeddings:
            # handle classifier being a Sequential -> get the final Linear out_features
            final_out = self.classifier[-1].out_features if isinstance(self.classifier, nn.Sequential) else self.classifier.out_features
            logits = torch.zeros(final_out).to(next(self.parameters()).device)
            if return_attn_weights:
                return logits, {}
            return logits
        ...
        if return_attn_weights:
            case_emb, case_weights, case_penalty = self.case_attention(
            ...
            # Aggregate regularization penalties from case/stain/patch levels
            total_reg = case_penalty
            for sn in stain_names:
                sinfo = stain_attention_weights.get(sn)
                if isinstance(sinfo, dict):
                    sp = sinfo.get('stain_penalty')
                    if sp is not None:
                        total_reg = total_reg + sp
                    pps = sinfo.get('patch_penalties')
                    if pps is not None:
                        for p in pps:
                            total_reg = total_reg + p
            all_weights['reg_loss'] = total_reg
