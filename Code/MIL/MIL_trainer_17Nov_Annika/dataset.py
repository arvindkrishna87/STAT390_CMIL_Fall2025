...
class StainBagCaseDataset(Dataset):
    ...
    def _load_slice_tensor(self, paths: List[str]) -> Optional[Tuple[torch.Tensor, List[str]]]:
        """
        ...
        if len(imgs) == 0:
            return None
        return torch.stack(imgs), used_paths  # (P, C, H, W), [paths]
    ...
    def __getitem__(self, idx: int) -> Dict[str, Any]:
        ...
        for stain in self.stains:
            ...
            paths_for_stain: List[List[str]] = []
            for sl in slice_lists:
                if not sl:
                    continue
                res = self._load_slice_tensor(sl)
                if res is None:
                    if self.drop_empty_slices:
                        continue
                    else:
                        continue
                sl_tensor, used_paths = res
                tensors_for_stain.append(sl_tensor)
                paths_for_stain.append(used_paths)
            
            stain_slices[stain] = tensors_for_stain
            # store parallel structure of paths (list of slices -> list of patch paths)
            stain_patch_paths = locals().get('stain_patch_paths', {})
            stain_patch_paths[stain] = paths_for_stain
            # save into a variable to return later
            try:
                _ = case_patch_paths
            except NameError:
                case_patch_paths = {}
            case_patch_paths[stain] = paths_for_stain
        
        return {
            "case_id": case_id,
            "stain_slices": stain_slices,  # dict[stain] -> list[Tensor(P,C,H,W)]
            "stain_patch_paths": case_patch_paths,  # dict[stain] -> list[list[path]] parallel to stain_slices
            "label": label,
        }
